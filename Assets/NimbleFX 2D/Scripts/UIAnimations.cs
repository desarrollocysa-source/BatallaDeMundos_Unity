...
namespace BhanuProductions.NimbleFX
{
    ...
    public class UIAnimations : MonoBehaviour
    {
        ...
        private RectTransform target => targettedSprite.GetComponent<RectTransform>();
        private Image targetedSprite => targettedSprite.GetComponent<Image>();
        ...
        public void StartShake(UnityEvent onShakeComplete = null)
        {
            timer = 0f;
            shaking = true;
        ...
        public void StartBounce(UnityEvent onBounceComplete = null)
        {
            timer = 0f;
            bouncing = true;
        ...
        public void StartGlow(UnityEvent onGlowComplete = null)
        {
            timer = 0f;
            glowing = true;
        ...
        public void StartRotation(UnityEvent onFinish = null)
        {
            isRotating = true;
            delayTimer = 0f;
            onComplete = onFinish;
        ...
        public void StartSpriteSheet(UnityEvent onFinish = null)
        {
        ...
        public void StartScale(UnityEvent onFinish = null)
        {
            ScaleOff = false;
            originalScale = target.localScale;
            scaleTimer = 0f;
        ...
        public void StartElasticResize(UnityEvent onFinish = null)
        {
            EROff = false;
        ...
        private void StartSelectedEffect(UnityEvent onFinish = null)
        {
            switch (animationType)
            {
                case AnimationType.SpriteSheetEffect:
                    if (!isPlayingSpriteSheet)
                        StartSpriteSheet(onFinish);
                    break;
                case AnimationType.BounceEffect:
                    if (!bouncing)
                        StartBounce(onFinish);
                    break;
                case AnimationType.GlowEffect:
                    if (!glowing)
                        StartGlow(onFinish);
                ...
                case AnimationType.RotateEffect:
                    if (!isRotating)
                        StartRotation(onFinish);
                    break;
                case AnimationType.ElasticResizing:
                    if (!isElasticResizing)
                        StartElasticResize(onFinish);
                    break;
            }
        }
        ...
        public void ExcuteAfterClick()
        {
            StartSelectedEffect(onComplete);
        }
